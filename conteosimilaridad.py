...
def imprimir_matriz(matriz, label):
    label.config(text="Matriz:\n" + str(matriz))
...
def comparar_archivos(archivo1, archivo2, resultado_text):
    try:
        ...
        imprimir_matriz(df1, matriz1_label)
        imprimir_matriz(df2, matriz2_label)
        num_elementos_iguales = 0
        tolerancia = 0.0001

        for i in range(len(df1.index)):
            for j in range(i, len(df1.columns)):
                if i < len(df1.index) and j < len(df1.columns):
                    ...
                    if isinstance(valor1, (int, float)) and isinstance(valor2, (int, float)):
                        ...
                        elif math.isclose(valor1, valor2, rel_tol=tolerancia):
                            num_elementos_iguales += 1
                            

        resultado_text.config(state=tk.NORMAL)
        resultado_text.delete(1.0, tk.END)
        resultado_text.insert(tk.END, f"El número de elementos iguales en la diagonal superior y por encima es: {num_elementos_iguales}")
...
matriz1_label = tk.Label(ventana, text="Matriz 1:")
matriz2_label = tk.Label(ventana, text="Matriz 2:")
...
matriz1_label.grid(row=3, column=0, columnspan=3, pady=5)
matriz2_label.grid(row=4, column=0, columnspan=3, pady=5)
resultado_label.grid(row=5, column=0, columnspan=3, pady=5)
resultado_text.grid(row=6, column=0, columnspan=3, pady=5)
